...
RSpec.describe CalculateWeeklyDisbursementsWorker do
  subject(:perform_worker) { described_class.new.perform }

  let(:merchant_1) { create(:merchant, :disbursed_weekly) }
  let(:merchant_2) { create(:merchant, :disbursed_weekly) }

  let!(:order_1) { create(:order, :undisbursed, merchant: merchant_1, created_at: 3.days.ago) }
  let!(:order_2) { create(:order, :undisbursed, merchant: merchant_1, created_at: 5.days.ago) }
  let!(:order_3) { create(:order, :undisbursed, merchant: merchant_1, created_at: 4.days.ago) }

  before do
    create(:order, :undisbursed, merchant: merchant_1, created_at: Date.current)
    create(:order, :undisbursed, merchant: merchant_1, created_at: 9.days.ago)
    create(:order, :disbursed, merchant: merchant_1, created_at: 2.days.ago)
    create(:order, :disbursed, merchant: merchant_1, created_at: 5.days.ago)

    create(:order, :undisbursed, merchant: merchant_2, created_at: 8.days.ago)
